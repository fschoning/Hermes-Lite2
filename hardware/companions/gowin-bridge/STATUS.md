...
| **The ROLE strap** | One 1x3 header with ONE shunt plus one inverting device, on both boards. `ROLE_N` carries a **10 k pull-UP** so a missing or dead inverter *disables* a host-facing port rather than enabling one; `ROLE` carries a 100 k pull-down so "no shunt" is a defined state and never a floating CMOS input. The inverter is **one N-channel MOSFET** - gate on `ROLE`, source to ground, drain on `ROLE_N` loaded by the 10 k pull-up that is fitted anyway: Alpha & Omega **AO3400A, LCSC C20917**, SOT-23, **Basic** tier, ~890,000 in stock, Vgs(th) 1.45 V maximum so a 3.3 V strap drives it with 1.85 V of margin. A single-gate logic inverter was the first choice and was dropped on cost: **no 74x1G04, 1G00, 1G02, 1G14 or 1G07 of any brand or package is a Basic part in JLCPCB's library**, so one gate would have cost the $3.07 per-unique-Extended-part fee. `COST.md` 5a examines what the MOSFET gives up - the short answer is nothing that matters for a static jumper level, and the fail-safe direction is unchanged. |
...
| Cost | Priced end to end for **2 and 3 assembled panels** in `README.md` section 8: **$196.58** and **$251.37** respectively, all in, including shipping to Germany and the two hand-soldered sockets. **`COST.md`** takes it apart against rev B line by line in the categories the fab bills, answers whether panelising saves money, separates what the third cable cost from what came from elsewhere, and ranks what could be cut. `tools/cost_model.py` does the arithmetic from the generated BOMs and PCBs, so the numbers cannot drift from the boards. |
...
| **Whether JLCPCB accepts this panel** | Their Economic assembly service states a panelisation rule of **mouse-bite separations only**; Standard allows mouse-bite or V-cut. This panel mixes three V-scores with one mouse-bite separation. | Ask the fab before paying. The fallback is two separate orders, which costs only about $9.70 more at two sets - see `COST.md`. |
